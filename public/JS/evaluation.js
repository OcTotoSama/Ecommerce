    ...
    document.querySelectorAll('.etoiles').forEach(container => {
        ...
        etoiles.forEach((etoile, index) => {
            ...
            etoile.addEventListener('click', () => {
                ...
                .then(r => r.json())
                .then(data => {
                    // ✅ Affiche le message d'erreur dans la console si échec
                    if (!data.success) {
                        console.error('Erreur évaluation:', data.message, data.file, data.line);
                        return;
                    }
                    ...
                    const label = document.querySelector(`.note-label-${produitId}`);
                    if (label) label.innerText = `Votre note : ${data.note}/5`;
